...
def retrieve(name: str):
  ...
  try:
    for header in code.find_all('div', class_= 'lister-item-content'):
      if header.h3.text == 'Titles':
        section = header
        break
    
    link = section.find('td', class_='result_text')
  #    if link.a.text == movie_put:
    reference_id = link.find('a')['href'].split('/')[2]
...
#df1 = pd.concat([df1, retrieve('Avatar')])

# --------------------> Testing ------------------------------------------
#for i in ['Primer', 'The Rise and Fall of Miss Thang']:
...
#base_url = 'https://www.imdb.com/find?ref_=nv_sr_fn&q={}&s=all'
#base_ = 'https://www.imdb.com/find?inception'

imdb_data = df1.to_json('imdb_data.json', orient='records')
